...
public final class Scrcpy {
    ...
    public static void startScrcpy(String... args) {
        started = true;
        try {
    ...
    private static void release() {
        ...
        // 5
        try {
            if (Integer.parseInt(Channel.execReadOutput("ps -ef | grep easycontrol.server | grep -v grep | grep -c 'easycontrol.server'").trim()) == 1) {
